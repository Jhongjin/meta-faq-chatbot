-- 안전한 인덱스 최적화 (2025-01-20)
-- PostgreSQL 버전 호환성 문제 해결

-- 1. 메모리 및 타임아웃 설정
SET maintenance_work_mem = '64MB';
SET work_mem = '16MB';
SET statement_timeout = '60s'; -- 60초 타임아웃

-- 2. 현재 상태 확인
SELECT 'Starting safe index optimization' as status, NOW() as timestamp;

-- 3. 현재 인덱스 상태 확인 (안전한 방식)
SELECT 
...
SELECT 'Vector index processing completed' as status, NOW() as timestamp;
...
SELECT 'Safe index optimization completed successfully' as status, NOW() as timestamp;
