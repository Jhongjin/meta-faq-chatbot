...
- 번호 매기기 시 1.을 반복하지 마세요.`;
    ...
    return `**답변 작성 규칙:**
아래 형식을 반드시 그대로 사용하세요. 섹션 제목은 변경하지 마세요.

---
### 핵심 요약
(질문 ${queryContext}에 대한 결론을 2줄 이내로 작성)

### 핵심 답변
(가장 중요한 내용을 불렛 포인트(-)로 작성. 문서에 명시된 내용만 포함)

### 상세 설명
(세부 내용을 작성. 절차·순서는 1, 2, 3... 번호를 사용. 범주가 바뀔 때만 소제목 추가)
...
**주의사항:**
- 본문 중간에 [출처 X] 같은 인라인 출처 표기 금지.
- 문서에 없는 내용은 절대 작성하지 마세요.`;
  }
