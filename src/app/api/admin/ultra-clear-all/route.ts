...
export async function DELETE(request: NextRequest) {
  try {
    console.log('🗑️ 초강력 데이터 전체 삭제 시작...');
    ...
    // 1. document_chunks 테이블 완전 삭제
    console.log('🗑️ document_chunks 테이블 완전 삭제 중...');
    
    // 배치 삭제 (한 번에 너무 많이 삭제하면 타임아웃 발생할 수 있음)
    let totalDeleted = 0;
    let batchSize = 1000;
    let hasMore = true;
    let attempts = 0;
    ...
    console.log(`✅ document_chunks 삭제 완료: 총 ${totalDeleted}개 삭제`);
    ...
    // 3. document_metadata 테이블 삭제 (테이블이 존재하는 경우에만)
    ...
    try {
      ...
      if (metaError) {
        console.log('⚠️ document_metadata 테이블이 존재하지 않거나 이미 비어있음');
      } else {
    ...
    return NextResponse.json({
      ...
      message: allEmpty 
        ...
        : `일부 데이터가 남아있습니다. (청크: ${finalChunks || 0}개)`,
      data: {
        finalCounts: {
    ...
    console.error('❌ 초강력 데이터 삭제 중 오류 발생:', error);
    return NextResponse.json(
      { 
        success: false, 
