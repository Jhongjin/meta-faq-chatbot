...
// Chat 페이지 테마 적용 (우측 패널 포인트 컬러 기반)
const chatTheme = {
  bgMain: "#10131a",
  bgSidebar: "#171c25",
  bgInput: "#151b24",
  ...
  accent: "#8ab4ff",
  accentSecondary: "#ff9ed7",
  highlight: "#7df3d3",
  rightPanelBg: "linear-gradient(180deg, #1f2533 0%, #262f45 60%, #1c2232 100%)",
  rightPanelCardBg: "linear-gradient(180deg, #293043 0%, #2f3750 100%)",
  positive: "#7ae397",
  warning: "#ffdf84",
};
...
function RightPanelPreview() {
  const recommendedQuestions = [
    "네이버 디스플레이 광고의 최근 업데이트 내용은 무엇인가요?",
    "네이버 광고 상품 소개서는 언제 최종 수정되었나요?",
    "가격 정책이 달라졌다면 적용 시점은 언제인가요?"
  ];
  
  const keyHighlights = [
    { label: "신뢰도", value: "85%", tone: chatTheme.positive },
    { label: "정확도", value: "92%", tone: chatTheme.accent },
    { label: "위험요소", value: "LOW", tone: chatTheme.warning },
  ];
  
  return (
    <div
      className="rounded-[32px] p-6 sm:p-8 shadow-[0_20px_60px_rgba(0,0,0,0.45)] border space-y-6"
      style={{ 
        background: chatTheme.rightPanelBg,
        borderColor: chatTheme.border
      }}
    >
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-xs uppercase tracking-[0.3em]" style={{ color: chatTheme.textSecondary }}>
            관련 자료
          </p>
          <h3 className="text-xl font-bold" style={{ color: chatTheme.textPrimary }}>
            질문과 관련된 문서 & 가이드라인
          </h3>
          <p className="text-sm mt-2" style={{ color: chatTheme.textSecondary }}>
            LLM 답변 이후, 신뢰도와 핵심 포인트를 즉시 제공합니다.
          </p>
        </div>
        <Badge className="text-xs px-3 py-1" style={{ backgroundColor: chatTheme.accentSecondary, color: "#111" }}>
          AI 분석
        </Badge>
      </div>

      <div className="rounded-2xl p-5 border space-y-3" style={{ background: chatTheme.rightPanelCardBg, borderColor: chatTheme.border }}>
        <div className="flex items-center justify-between">
          <div>
            <p className="text-sm" style={{ color: chatTheme.textSecondary }}>답변 핵심 요약</p>
            <h4 className="text-lg font-semibold" style={{ color: chatTheme.textPrimary }}>네이버 디스플레이 상품 업데이트</h4>
          </div>
          <span className="text-sm font-semibold" style={{ color: chatTheme.accent }}>
            분석 중...
          </span>
        </div>
        <div className="grid grid-cols-3 gap-3">
          {keyHighlights.map((item) => (
            <div key={item.label} className="rounded-xl p-3 text-center" style={{ backgroundColor: "#ffffff0b", border: `1px solid ${chatTheme.border}` }}>
              <p className="text-xs tracking-[0.2em]" style={{ color: chatTheme.textSecondary }}>
                {item.label}
              </p>
              <p className="text-lg font-bold" style={{ color: item.tone }}>
                {item.value}
              </p>
            </div>
          ))}
      ...
      <div className="rounded-2xl p-5 border space-y-4" style={{ background: chatTheme.rightPanelCardBg, borderColor: chatTheme.border }}>
        <div className="flex items-center justify-between">
          <div>
            <p className="text-sm" style={{ color: chatTheme.textSecondary }}>참고 문서 핵심 내용</p>
            <p className="text-base font-semibold" style={{ color: chatTheme.textPrimary }}>
              Naver_Display_AD_통합상품소개서 (1페이지)
            </p>
          </div>
          <Button size="sm" style={{ backgroundColor: chatTheme.accent, color: "#0a0d14" }}>
            <FileText className="w-3 h-3 mr-1" />
            문서 열기
          </Button>
        </div>
        <div className="rounded-xl bg-black/20 p-3 text-xs" style={{ color: chatTheme.textSecondary }}>
          마지막 업데이트: 2025.11.21 · 문서 신뢰도 92% · 문서 기반 요약 3건 제공
        </div>
      </div>

      <div className="rounded-2xl p-5 border space-y-3" style={{ background: chatTheme.rightPanelCardBg, borderColor: chatTheme.border }}>
        <div className="flex items-center gap-2">
          <MessageSquare className="w-4 h-4" style={{ color: chatTheme.accent }} />
          <p className="text-sm font-semibold" style={{ color: chatTheme.textPrimary }}>
            관련 질문 예측
          </p>
          <Badge variant="secondary" className="text-[11px] px-2 py-0.5" style={{ backgroundColor: "#ffffff12", color: chatTheme.textSecondary }}>
            AI 추천
          </Badge>
        </div>
        <ul className="space-y-2">
          {recommendedQuestions.map((question) => (
            <li
              key={question}
              className="flex items-start gap-2 text-sm p-2 rounded-xl transition-colors duration-200"
              style={{ color: chatTheme.textSecondary }}
            >
              <span className="w-1.5 h-1.5 rounded-full mt-2" style={{ backgroundColor: chatTheme.highlight }} />
              <span className="flex-1">{question}</span>
              <ArrowRight className="w-4 h-4" style={{ color: chatTheme.textSecondary }} />
            </li>
          ))}
        </ul>
      </div>
  ...
  return (
    ...
    >
      <MainLayout>
        ...
        {/* Right Panel Inspired Section */}
        <section 
          className="w-full py-12 sm:py-16"
          style={{ 
            background: "radial-gradient(circle at top, rgba(138,180,255,0.15), transparent 70%)"
          }}
        >
          <div className="max-w-6xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
            <div className="space-y-6">
              <Badge className="w-fit" style={{ backgroundColor: chatTheme.accentSecondary, color: "#0f1015" }}>
                Right Panel Palette
              </Badge>
              <h3 className="text-3xl sm:text-4xl font-bold leading-tight" style={{ color: chatTheme.textPrimary }}>
                메인 페이지에서도 우측 패널의 감성을 살린 디자인 실험
              </h3>
              <p className="text-base sm:text-lg leading-relaxed" style={{ color: chatTheme.textSecondary }}>
                LLM 답변 옆에서 정보를 정리해 주는 우측 패널의 하이라이트 색상을 메인 페이지에 적용했습니다. 
                다층 그라데이션과 포인트 칼라로 중요한 정보를 강조하고, 카드마다 명확한 대비를 주어 시인성을 확보했습니다.
              </p>
              <ul className="space-y-3">
                {[
                  "신뢰도·정확도 배지를 메인 정보 카드에도 이식",
                  "하이라이트 컬러(민트/핑크)로 CTA와 상태를 직관적으로 표현",
                  "우측 패널 모듈을 그대로 가져온 미니 대시보드 구성"
                ].map((item) => (
                  <li key={item} className="flex items-start gap-3">
                    <span className="mt-2 w-2 h-2 rounded-full" style={{ backgroundColor: chatTheme.highlight }} />
                    <span style={{ color: chatTheme.textSecondary }}>{item}</span>
                  </li>
                ))}
              </ul>
            </div>
