  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center px-4">
      ...
      >
        ...
        <div className="relative p-8 md:p-10 flex-1 overflow-y-auto">
          <button
            onClick={onClose}
    ...
    <div className="min-h-screen bg-[#0B0F17] text-white font-sans selection:bg-blue-500/30 overflow-x-hidden" style={{ fontFamily: "'Noto Sans KR', sans-serif" }}>
      ...
      <nav
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
          isScrolled ? "bg-[#0B0F17]/80 backdrop-blur-md border-b border-white/5 py-4" : "bg-transparent py-6"
        }`}
      >
        <div className="max-w-7xl mx-auto px-6 flex items-center justify-between relative">
          <Link href="/test/main2" className="flex items-center gap-2 group z-10">
            <motion.div
          ...
          {/* Mobile Menu Toggle */}
          <button
            className="md:hidden text-gray-400 hover:text-white z-10"
          ...
          >
            <div className="flex flex-col gap-6">
              ...
              <button 
                ...
                className="text-lg text-gray-300 text-left flex items-center justify-between"
              >
                Solutions
              ...
              {showSolutionsDropdown && (
                <div className="ml-4 space-y-2">
                  <Link href="#" className="block text-sm text-gray-400 hover:text-white">Ad Guidelines</Link>
                  <Link href="#" className="block text-sm text-gray-400 hover:text-white">Ad Insights</Link>
                  <Link href="#" className="block text-sm text-gray-400 hover:text-white">Ad Capture Archive</Link>
                  <Link href="#" className="block text-sm text-gray-400 hover:text-white">Ad Design Studio</Link>
                </div>
              )}
              <button 
                onClick={() => {
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col items-center text-center mb-16">
            ...
            >
              ...
              <form onSubmit={handleChatSubmit} className="relative bg-[#131823] border border-white/10 rounded-2xl p-2 flex items-center shadow-2xl">
                ...
                <Input
                  ...
                  className="border-0 bg-transparent text-lg py-6 text-white placeholder:text-gray-500 focus-visible:ring-0 flex-1"
                  value={chatInput}
                ...
                <Button 
                  ...
                  className="h-12 px-8 rounded-xl bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-500 hover:to-purple-500 text-white font-medium shadow-lg shadow-blue-500/20 transition-all duration-300 hover:scale-105 disabled:opacity-50"
                >
                  {isLoading ? (
            ...
            <div className="max-w-7xl mx-auto px-6 flex items-center justify-between text-gray-500">
              ...
              <div className="flex-1 flex justify-around items-center opacity-50 grayscale hover:grayscale-0 transition-all duration-500">
                {VENDORS.map((vendor, i) => {
                  ...
                  const brandColor = vendorColors[vendor];
                  const iconColor = vendor === "X(Twitter)" ? "#FFFFFF" : brandColor;
                  ...
                  return (
                    <div key={i} className="flex items-center gap-2 group cursor-pointer hover:opacity-100 transition-opacity">
                      ...
                      <span className="font-semibold group-hover:text-white transition-colors">{vendor}</span>
                    </div>
                  );
                })}
              </div>
          ...
          <div className="mb-32">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {vendorUpdatesLoading ? (
                [...Array(3)].map((_, i) => (
                  <div key={i} className="p-6 rounded-2xl bg-[#131823] border border-white/5">
                    <Skeleton className="h-6 w-24 mb-4 bg-gray-700/50" />
                    <Skeleton className="h-4 w-full mb-2 bg-gray-700/50" />
                    <Skeleton className="h-4 w-4/5 bg-gray-700/50" />
                  </div>
                ))
              ) : vendorUpdatesError ? (
                <div className="col-span-3 p-6 rounded-2xl bg-red-500/10 border border-red-500/20 text-center">
                  <p className="text-red-400">업데이트 정보를 불러올 수 없습니다.</p>
                </div>
              ) : vendorUpdatesData?.updatesWithDate && vendorUpdatesData.updatesWithDate.length > 0 ? (
                vendorUpdatesData.updatesWithDate.slice(0, 3).map((update, i) => {
                  const vendorIcon = vendorIcons[update.vendor];
                  const vendorColor = vendorColors[update.vendor];
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center justify-between mb-4">
                        <Badge variant="outline" className="border-white/10 text-gray-400 flex items-center gap-2">
                          {vendorIcon && (
                            <span 
                        ...
                        <span className="text-xs text-gray-500">{update.formattedDate || new Date().toLocaleDateString('ko-KR')}</span>
                      </div>
                ...
                <div className="col-span-3 p-6 rounded-2xl bg-[#131823] border border-white/5 text-center">
                  <p className="text-gray-400">업데이트 정보를 불러오는 중입니다...</p>
                </div>
              )}
            </div>
          </div>
          ...
          >
            <div className="text-center mb-12">
              <h2 className="text-3xl font-bold mb-4">실시간 성능 지표</h2>
              <p className="text-gray-400">시스템 성능과 사용자 만족도를 실시간으로 확인하세요</p>
            </div>
            
            <Card className="bg-[#131823] border-white/5 group">
                    ...
                    <Table className="min-w-[600px]">
                      <TableHeader>
                        <TableRow className="border-white/20">
                          <TableHead className="font-semibold text-white">지표</TableHead>
                          <TableHead className="font-semibold text-white">현재 값</TableHead>
                          <TableHead className="font-semibold text-white hidden sm:table-cell">변화율</TableHead>
                          <TableHead className="font-semibold text-white">상태</TableHead>
                        </TableRow>
                      </TableHeader>
                      <TableBody>
                        {performanceData.map((item, index) => (
                          <TableRow key={index} className="border-white/10 hover:bg-white/5">
                            <TableCell className="font-medium text-gray-300">{item.metric}</TableCell>
                            <TableCell className="font-semibold text-white">{item.value}</TableCell>
                            <TableCell className="text-green-300 hidden sm:table-cell">{item.trend}</TableCell>
                            <TableCell>
                              <Badge 
          ...
          <div className="relative rounded-3xl overflow-hidden p-12 text-center">
            ...
            <div className="relative z-10 max-w-3xl mx-auto">
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
                ...
                <Button 
                  variant="outline" 
                  className="h-14 px-8 text-lg rounded-full border-white/20 text-white hover:bg-white/10 w-full sm:w-auto"
                  onClick={() => {
                    setAuthModalMode("signup");
                    setAuthModalOpen(true);
                  }}
      ...
      <AnimatePresence>
        {showFeaturesModal && (
          <TechSpecsModal onClose={() => setShowFeaturesModal(false)} />
        )}
      </AnimatePresence>
