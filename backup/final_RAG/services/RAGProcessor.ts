...
export class RAGProcessor {
  ...
  async saveDocumentToDatabase(document: DocumentData): Promise<void> {
    try {
      ...
      // PDF 파일의 경우 content 필드를 비워두고 placeholder 사용
      let cleanContent = '';
      if (document.type === 'pdf') {
        cleanContent = `[PDF 파일: ${document.title}] - 텍스트 추출은 클라이언트에서 처리됩니다.`;
      } else {
        cleanContent = document.content ? document.content.replace(/\0/g, '') : '';
      }
