...
export default function NewDocumentUpload({ onUpload }: NewDocumentUploadProps) {
  ...
  const handleFileSelect = (selectedFiles: File[]) => {
    ...
    const validFiles = selectedFiles.filter(file => {
      ...
      // 파일 크기 제한 (20MB)
      const maxFileSize = parseInt(process.env.NEXT_PUBLIC_MAX_FILE_SIZE || '20971520'); // 20MB
      ...
      if (!isValidSize) {
        toast({
          title: "파일 크기 초과",
          description: `${file.name} 파일은 ${Math.round(maxFileSize / 1024 / 1024)}MB를 초과할 수 없습니다. 최대 20MB까지 업로드 가능합니다.`,
          variant: "destructive",
        });
      }
