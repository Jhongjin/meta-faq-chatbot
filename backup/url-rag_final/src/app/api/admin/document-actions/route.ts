...
async function handleDownload(documentId: string) {
  try {
    ...
    // 원본 파일 데이터가 있는 경우
    const base64Data = fileData.split(',')[1]; // data:application/pdf;base64, 부분 제거
    ...
    if (document.type === 'url' && document.url) {
      ...
      try {
        ...
        if (documentData.content) {
          ...
          // content 필드를 사용하여 재인덱싱
          const content = documentData.content;
          const blob = new Blob([content], { type: documentData.file_type || 'text/plain' });
          
          // File 객체 생성
          const file = new File([blob], document.title, { 
            type: documentData.file_type || 'text/plain' 
          });
